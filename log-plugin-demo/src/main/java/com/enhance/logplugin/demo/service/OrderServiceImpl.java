...
public class OrderServiceImpl implements AopProxy<OrderService>, OrderService {
  ...
  private final LogTraceProperty logTraceProperty;
  ...
  public List<Order> listOrder(Order order) {
    List<String> uri = logTraceProperty.getUri();
  ...
  @Transactional
  @Log(
      itemIds = {"orderDetailDTO.orderCode"},
      itemType = "order表",
      action = Action.U)
  ...
  @LogProfiler(
      excludeInParam = {"arg2", "arg1.userId"},
      includeInParam = "arg0.orderCode")
  @Override
  public Order handelUpdate(
      OrderDetailDTO orderDetailDTO, UserDTO userDTO, List<OrderEntryDetail> orderEntryDetails) {
    return null;
  }
  ...
  public void conversionOrderEntry(Order order, OrderDetailDTO detailDTO) {
    ...
    if (ListUtil.listIsNotEmpty(orderEntries, "不为空")) {
      ArrayList<OrderEntryDetail> orderEntryArrayList =
          Lists.newArrayListWithCapacity(orderEntries.size());
      for (OrderEntry orderEntry : orderEntries) {
        ...
        // ===============================================================================
        LogUtil.printCodeByTemplate(
            "skuId:{} 数量:{}", orderEntry.getSkuId(), orderEntry.getQuantity());
        log.info("开始处理...");
        log.info("处理中");
        log.info("处理结束");
        LogUtil.clearMDC();
